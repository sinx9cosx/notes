...
要开始非辐射计算，您需要一个`*.dat`文件、一个`*.nac`文件、一个MOMAP控制文件，以及一个可选的并行控制文件。`*.dat`文件是evc结果文件。`*.nac`是NACME计算结果文件。与所有MOMAP计算一样，TVCORF_IC和TVSPEC_IC子程序需要一个MOMAP控制文件。并行控制文件用于控制将使用多少个计算进程。
