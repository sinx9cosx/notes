...
全流程概述：Gaussian 上游准备几何/频率/能量/偶极 → evc 耦合两态振动学 → 分两路算 kr 与 kic → 合成 Φ、τ 与光谱，对照文献。
...
通常高激发态通过快速内转换弛豫至 S1，再从 S1 发光（Kasha 规则）。DHR 从高态（S9）直接发光，违反这条经验规则，叫**反-Kasha**。要解释它，就需分别计算 S1 与 S9 的 kr 和 kic，比较各失活通道的速率——这正是本流程的用途。
...
- 每个 opt freq 的 .log 旁必须放同名 .fchk，MOMAP 才能读取。
- Ead = 两态各自优化几何下电子总能量差的绝对值，决定谱的位置；EDMA/EDME 是吸收/发射偶极矩，决定谱强度与速率量级。`Dip. S.` 输出的是 μ²，要开根号再乘 2.5417 才能换算为 Debye（1 a.u. = 2.5417 D）。
...
**本步做什么。** 读两个 opt freq 的 log+fchk，通过 Duschinsky 变换（旋转与平移）将两个电子态的简正坐标相互关联，进而计算各模式的简正模式位移、Huang-Rhys 因子与总重组能。
...
1. cart 与 dint 的 `Total reorganization energy` 差 <1000 cm⁻¹ → 后续用 cart（`DSFile=evc.cart.dat`），否则用 dint。
	...
	- Reorganization Energy 2 = 激发态重组能 λ₂：分子站在基态平衡几何上时，激发态势能面比自己的平衡点高出的能量。物理图像：吸收光子后分子垂直激发到 Franck-Condon 点，激发态随后弛豫到自己平衡几何，释放的能量就是 λ₂。它决定吸收谱的蓝移：垂直吸收能 = Ead + λ₂喵。
...
**本步做什么。** 将 evc 的振动信息与 Ead（决定谱的位置）、EDMA/EDME（决定谱强度）结合，计算发射相关函数，经傅里叶变换得到荧光光谱；对谱按能量三次方加权积分得到 kr。相关函数是跃迁偶极矩的时间关联函数，其振荡编码了振动结构信息。
...
| EDMA / EDME | 吸收/发射偶极矩（Debye），决定谱强度与速率 | 6.93 / 7.25 |
| Temp | 温度（K） | 300 |
...
> DHR 的 S1 振子强度较大（"强发射"），但 kic 比 kr 大约 200 倍，绝大多数激发态分子经无辐射通道失活，Φ 仅 0.5%。"强发射"指辐射通道本身强（f 大），最终发光效率由 Φ 决定。
