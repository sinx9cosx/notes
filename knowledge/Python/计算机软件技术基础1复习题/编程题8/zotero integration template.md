...
## Annotations
{% endif %}{% set newAnnotations = annotations | filterby("date","dateafter", lastlmportDate) %}{% if newAnnotations. length > 0 %}
#### Imported: {{importDate | format("YYYY-MM-DD h:mm a")}}
{% for a in newAnnotations %}
{% if a.annotatedText %}> {{la.annotatedText}} [Page{{a, page}}].({{a.attachment.select | replace("select", "open-pdf")}}?page={{a.page}}).
{% endif %}{% if a.comment %}> {{a.comment}}
{% endif %}{% if a.type == "image" %}> [Page{{a.page}}].({{a.attachment.select | replace("select", "open-pdf')}}?page=fa,page}})!
[[{{a.imageBaseName}}
(% endif %/(% endfor %)
(% endif %(% endpersist %
