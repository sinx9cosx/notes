...
> 位移后原暗态（133→136）与亮态混合：第二次优化跟踪的是混合态（135→137 主导，f≈0.22），前三个态中已无 133→136 主导的态（该成分主要转移至 ES4，3.0938 eV）。-224 cm⁻¹ 虚频来自单独 freq 作业（无态跟踪，按能量升序编号）对第 3 态即亮态（134→136）的频率计算，与优化跟踪的混合态不是同一个态。